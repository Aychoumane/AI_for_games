...
struct bt_t {
  ...
  double playout_mcts (int _color); 
  //void test_playout(int nb_partie);
...
double bt_t::playout_mcts (int _color){
  while(endgame() == EMPTY){
    bt_move_t single_move = get_rand_move() ; 
    if(can_play(single_move)){
          play(single_move);
      }
  }
  //fprintf(stderr, "Endgame: Le score est : %d\n",endgame() );
  return score(_color); 
