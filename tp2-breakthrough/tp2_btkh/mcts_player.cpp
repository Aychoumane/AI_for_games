...
char playername[128] = "GODZILLA_PLAYER" ; 
...
bt_move_t eval_MC(bt_t plateau, int nb_essai){
  plateau.update_moves() ; 
  bt_move_t best_move = plateau.moves[0];
  int max = -1;
  int nb_win ; 
  bt_t copie ; 
  int color = plateau.turn % 2 == 0 ? WHITE : BLACK ; 

  for(int i = 0; i<plateau.nb_moves; i++){
    copie = plateau ; 
    copie.play(copie.moves[i]);
    nb_win = 0 ; 

    for(int j = 0 ; j < nb_essai ; j ++ ){
      if(copie.playout_mcts(color) == 1){
        nb_win++ ; 
      }
    }

    if(nb_win > max ){
      best_move = plateau.moves[i]; 
      max = nb_win ; 
    }
  }
  return best_move ; 
...
void test_playout(bt_t plateau , int nb_partie){
  bt_t copie ; 
  int couleur ; 
  //int score ; 
  for(int i = 0 ; i< nb_partie ; i++){
    ...
    copie = plateau ; 
    copie.print_board(stderr) ; 
    copie.turn%2 == 0 ? couleur = WHITE : couleur = BLACK ;  
    //score = copie.playout_mcts(couleur) ; 
    copie.print_board(stderr) ;
    //printf("Le score est de %d pour les %d\n", score, couleur) ; 
  }
}
...
int main(int _ac, char** _av) {
  ...
  for (std::string line; std::getline(std::cin, line);) {
    ...
    else if(line.compare("playout") == 0) { cmd_ok=true; test_playout(B,50);}
    else if(line.compare("gmcmove") == 0) { cmd_ok=true; genmove_MC();}
    ...
    if(B.alternate_or_simultaneous == 0) {
      if(line.compare("genmove") == 0) { cmd_ok=true; genmove_MC();}
